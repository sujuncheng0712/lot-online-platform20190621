...
import {Input, Button, Icon, message, List, Popconfirm, Popover, Divider, Select, Row, Col, Badge} from 'antd';
        ...
        res.json().then((info) => {
          if (info.status) {
            ...
            info.data.forEach((val) => {
              if ((val.type === 1 || val.type === 4) && val.state === 4) {
                val.id = k;
                lists.push(val);
                k++;
              }
                ...
                <div key={item.uuid} style={styles.item}>
                  <div style={styles.rowT}>
                    <div style={{display: 'flex'}}>
                      ...
                      <div hidden={!item.link_id}>
                        <Divider type="vertical" />
                        <Badge status="success" text={`激活设备：${item.link_id}`} />
                        <Divider type="vertical" />
                        <Badge status="success" text={`激活时间：${item.link_at}`} />
                      </div>
                    </div>
                  ...
                  <div style={styles.row}>
                    ...
                    <div span={4} style={styles.pay_amount}>
                      <span style={{color: stateBadge[item.state]}}>
                        {(item.message === 'book' && item.state === 3) ? '已预约' : stateMap[item.state]}
                      </span>
                    </div>
