...
class DealerAdd extends PureComponent {
  ...
  render() {
    ...
    const merchantsList = [];
    lists.forEach((val) => {
      if (val.type === 1) merchantsList.push(<Select.Option key={val.uuid}>{val.contact}（{val.area}）</Select.Option>);
    });
    ...
    return (
      <PageHeaderLayout title={`${mid ? '修改' : '添加'}客户`}>
        ...
        <div style={{display: `flex`, alignItems: `center`, flexDirection: `row-reverse`}}>
          <Button type="primary" onClick={validate}>提交资料</Button>
        </div>
      </PageHeaderLayout>
    );
  }
