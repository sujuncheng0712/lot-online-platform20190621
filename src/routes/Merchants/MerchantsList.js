      ...
      {title: '代理区域', dataIndex: 'area', align: 'center'},
      {
        ...
        render: (val, infos) => (
          <Button.Group size="small">
            <Button onClick={() => {location.hash = `/merchants/merchants-profile/?mid=${val}`;}}>
              详情
            </Button>
            <Button
              hidden={!(localStorage.getItem("antd-pro-authority") === "vendors") || false}
              onClick={() => {location.hash = `/merchants/add-edit-merchants/?mid=${val}`;}}
            >
              编辑
            </Button>
            {infos.state !== 0 ? (
        ...
        <div style={{padding: 20, backgroundColor: '#fff'}}>
          <div hidden={!(localStorage.getItem("antd-pro-authority") === "vendors") || false}>
            <Button
              ...
              onClick={() => {location.hash = '/merchants/add-edit-merchants';}}
            >
              <Icon type="plus" /> 添加商家
            </Button>
            <Radio.Group
